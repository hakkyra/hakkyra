...
Hasura REST endpoints reference named queries stored in query collections. The acme metadata has 57 queries in the `allowed-queries` collection and 50+ REST endpoints mapping URL paths to those queries.
...
- [x] Config loader: load query collections and REST endpoints from acme-style fixtures
...
Hasura supports `disabled_for_roles` to block schema introspection for specific roles. The acme metadata has `disabled_for_roles: []` (allow all).
...
Hasura v2.28+ supports native queries (raw SQL exposed as GraphQL fields) with logical models (custom return types). The acme metadata has 3 native queries with 3 logical models.
...
Hasura supports `query_root_fields` and `subscription_root_fields` in select permissions to control which root fields a role can access. In acme, the `player` role uses empty arrays `[]` on bonus/bonus_currency/campaign tables to hide them from direct querying while still allowing access through relationships.
...
Schema introspection comparison of Hakkyra (localhost:8081) vs Hasura (localhost:8080) against the same acme database. Hakkyra has 2637 object types vs Hasura's 1805. The extra types come from Hakkyra auto-tracking all DB tables instead of only metadata-configured ones.
...
Schema comparison (hakkyra vs Hasura on acme DB) found missing comparison operators:
...
Second-round schema introspection comparison of Hakkyra (localhost:8081) vs Hasura (localhost:8080) against the acme production database. Types: hakkyra=4647 vs hasura=4571. Identifies remaining differences after Phase 9 fixes.
...
Third-round schema comparison of Hakkyra SDL (localhost:8081/sdl) vs Hasura introspection (localhost:8080) against the acme database. Types: hakkyra ~2,140 object/input types vs hasura ~4,571. Identifies regressions from P10 fixes and new gaps.
...
**Investigation result (updated 2026-03-16)**: Previous conclusion of "DB mismatch" was WRONG. Both services connect to the same DB (`postgresql://...@localhost:5432/acme`), all 50 tables and all columns exist. Re-verified via direct DB queries and introspection debug logging.
...
Fourth-round comparison using full introspection queries against both services on the same acme DB (`localhost:5432/acme`). Admin-level comparison plus **backoffice role** comparison (eliminates admin-only column noise).
